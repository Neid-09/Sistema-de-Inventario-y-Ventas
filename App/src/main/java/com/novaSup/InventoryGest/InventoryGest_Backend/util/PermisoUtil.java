...
public class PermisoUtil {
    ...
    * PERO EL VENDEDOR SE LIMITA A ESTO:
    ...
    public PermisoUtil(UsuarioServiceImpl usuarioService) {
        PermisoUtil.usuarioService = usuarioService;
    }
     ...
     * @return ResponseEntity con error si no tiene permiso, null si tiene permiso
     */
    public static ResponseEntity<?> verificarPermiso(Integer idUsuario, String nombrePermiso) {
        try {
            Usuario usuario = usuarioService.obtenerUsuarioPorId(idUsuario);

            // Comprobar si el usuario es administrador usando comparación insensible a mayúsculas
            if (usuario.getRol() != null &&
                    "Administrador".equalsIgnoreCase(usuario.getRol().getNombre())) {
                return null; // El administrador tiene permiso automáticamente
            }

            // Para usuarios no administradores, verificar el permiso específico
            if (!usuarioService.tienePermiso(usuario, nombrePermiso)) {
                return ResponseEntity.status(403).body("No tienes permisos para " + nombrePermiso);
            }
            return null; // Usuario tiene permiso
